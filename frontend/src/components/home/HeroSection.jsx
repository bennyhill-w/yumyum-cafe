    ...
    cards: [
      {
        ...
        title: "SWEET TREATS",
        text: "Made to Delight!",
        position: "top-[24%] left-[2%] lg:left-[-4%]",
      },
      {
        id: "hearty",
        icon: <FiStar size={13} />,
        title: "HEARTY MEALS",
        text: "Fresh. Filling. Always Loved.",
        position: "top-[58%] left-[0%] lg:left-[-6%]",
      },
      {
        id: "satisfying",
        icon: <FiStar size={13} />,
        title: "100% SATISFYING",
        text: "Big on Taste, Bigger on Quality!",
...
// One annotation card, positioned absolutely around the composite image
function AnnotationCard({ card, index }) {
  return (
    <motion.div
    ...
    >
      <motion.div
        ...
        className="bg-white/95 backdrop-blur-md rounded-2xl shadow-xl px-3.5 py-2.5 border border-gray-100/50 max-w-[150px]"
      >
        <div className="flex items-center gap-1.5 mb-1">
          <span className="text-brand-red flex-shrink-0">{card.icon}</span>
          <p className="text-gray-900 text-[10px] font-bold font-sans tracking-wide leading-none">
            {card.title}
          </p>
        </div>
        <p className="text-gray-500 text-[10px] font-sans leading-snug">
          {card.text}
        </p>
      </motion.div>
    </motion.div>
...
export default function HeroSection() {
  ...
  return (
    <section
      ...
      style={{
        background: `
          radial-gradient(ellipse 80% 60% at 65% 15%, #960101 0%, transparent 55%),
          radial-gradient(ellipse 70% 50% at 50% 100%, #5F0101 0%, transparent 60%),
          #460000
        `,
      }}
    >
      ...
      <motion.div
        ...
        style={{
          background:
            "radial-gradient(circle, rgba(253,211,77,0.08), transparent 70%)",
        }}
      />
        ...
        >
          ...
          >
            ...
            <motion.div
              ...
              className="absolute -top-2 right-0 lg:-right-4 z-30 hidden sm:block"
            >
              <motion.div
            ...
            >
              ...
              <AnimatePresence mode="wait">
                <motion.img
                  ...
                  alt="Yum-Yum Cafe signature meals"
                  initial={{ opacity: 0, scale: 1.04, y: 12 }}
                  animate={{ opacity: 1, scale: 1, y: 0 }}
                  exit={{ opacity: 0, scale: 0.97, y: -12 }}
                  transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
                  className="w-[145%] h-[145%] max-w-none relative z-10"
                  style={{
                    objectFit: "contain",
                    ...
                    WebkitMaskImage:
                      "radial-gradient(ellipse 62% 52% at 50% 50%, black 35%, transparent 88%)",
                    maskImage:
                      "radial-gradient(ellipse 62% 52% at 50% 50%, black 35%, transparent 88%)",
                  }}
                  loading="eager"
              ...
              {/* Per-slide annotation cards */}
              <AnimatePresence mode="wait">
                <div key={`cards-${currentSlide.id}`}>
                  {currentSlide.cards.map((card, i) => (
                    <AnnotationCard key={card.id} card={card} index={i} />
                  ))}
                </div>
              </AnimatePresence>
            </div>
