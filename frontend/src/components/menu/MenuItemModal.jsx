          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="flex items-center gap-3 mb-5">
              <div className="flex items-center gap-1 bg-brand-gold-light px-3 py-1.5 rounded-full">
                {Array.from({ length: 5 }).map((_, i) => (
                  <AiFillStar key={i} className="text-brand-gold-mid" size={14} />
                ))}
                <span className="text-brand-gold font-bold text-sm font-sans ml-1">4.8</span>
              </div>
              <span className="text-gray-400 text-sm font-sans">2,400+ ratings</span>
              <div className="flex items-center gap-1 ml-auto bg-green-50 px-3 py-1.5 rounded-full">
                <div className="w-1.5 h-1.5 rounded-full bg-green-500" />
                <span className="text-green-600 text-xs font-bold font-sans">Fresh today</span>
              </div>
            </div>

            {/* Description */}
            <div className="mb-5">
              <h3 className="font-display font-bold text-gray-900 text-lg mb-2">About this dish</h3>
            ...
            <div className="flex flex-wrap gap-2 mb-5">
              {[
                ...
                { icon: <HiOutlineSparkles size={13} />, text: 'No Preservatives' },
                { icon: <MdOutlineDeliveryDining size={13} />, text: 'Order Online' },
              ].map(h => (
                <span key={h.text} className="inline-flex items-center gap-1.5 bg-brand-red-light text-brand-red text-xs font-bold px-3 py-1.5 rounded-full font-sans">
                  {h.icon} {h.text}
                </span>
              ))}
            </div>
